...
void CpuSpec::importData(std::ifstream& file) {
    ...
    getline(file, model);
    file >> cores >> frequency >> threads >> cacheSize;
    file.ignore(); // Ignore the newline after reading integers
...
void GpuSpec::importData(std::ifstream& file) {
    ...
    getline(file, model);
    file >> memorySize >> coreClock >> memoryType >> shaderCores >> tdp;
    file.ignore();
}
